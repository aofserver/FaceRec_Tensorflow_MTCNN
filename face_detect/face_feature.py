...
class FaceFeature(object):
    def __init__(self, face_rec_graph, model_path = 'face_detect/models/20170512-110547.pb'):
    #def __init__(self, face_rec_graph, model_path = 'face_detect/models/inference.pb'):    
        '''
    ...
    def __load_model(self, model):
        ...
        if os.path.isfile(model_exp):
            ...
            with tf.gfile.GFile(model_exp, 'rb') as file_:  # For 1.13   
...
def prewhiten(x):
    mean = np.mean(x)
    std = np.std(x)
    # print("___________1.0 / np.sqrt(x.size)_____________",1.0 / np.sqrt(x.size))   #### Constant
    # print("___________std_____________",std,std>(1.0 / np.sqrt(x.size)))
    #   
